...
export default function SubmissionsScreen() {
  ...
  const { submissions, fetchSubmissions, rateSubmission } = useTaskStore();
  ...
  const loadSubmissions = async () => {
    try {
      // Barcha guruhlardagi vazifalarni olish
      for (const group of groups) {
        console.log('Fetching tasks for group:', group.id);
        await useTaskStore.getState().fetchTasks(group.id);
      }
      ...
      // Har bir vazifa uchun javoblarni olish
      for (const task of tasks) {
        console.log('Fetching submissions for task:', task.id);
